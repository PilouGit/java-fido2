 ...
 * @since 0.8.0
 */
public class AuthenticationCommandExample3 extends BaseExample {
  ...
  public static void main(String[] args) throws HidException {
    ...
    AuthenticationCommandExample3 example = new AuthenticationCommandExample3();
  ...
  private boolean handleInitialise(HidDevice hidDevice) {
    ...
    int bytesWritten = hidDevice.write(request.getPacket().get(0).getPacket(), CTAP_MAX_REPORT_LEN, (byte) 0x00, true);
    if (bytesWritten < 0) {
  ...
  public void hidDataReceived(HidServicesEvent event) {
    ...
    HIDInitializationPacket response=new HIDInitializationPacket(initialiseResponse);
    if (state==0) {
    	// CTAPHIDInitResponse responseCmd=new CTAPHIDInitResponse(response);
    	//  System.out.println(responseCmd);
      ...
      // 		);
      // int bytesWritten = event.getHidDevice().write(requestCbor.getPacket().get(0).getPacket(), CTAP_MAX_REPORT_LEN, (byte) 0x00, true);
       state=1;
    }else
    if (state==1)
    {
